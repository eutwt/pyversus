...
DuckDB- and Spark-backed tools for comparing two relations (tables or
views), pandas/polars DataFrames, or PySpark DataFrames without
materializing the full inputs in Python.

`versus.compare()` stays the single public entry point. DuckDB remains
the default backend, and Spark is selected automatically when Spark
inputs are involved.
...
Spark-backed comparisons
------------------------

When you pass PySpark inputs, the API stays the same and the backend
switches automatically. Helper methods return Spark DataFrames, so use
`.show()` to inspect them.
...
If either input is a Spark DataFrame, `compare()` dispatches to Spark
and converts the other supported inputs as needed.
